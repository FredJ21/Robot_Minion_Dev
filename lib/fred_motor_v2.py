...
CONF_SERIAL_PORT_1 = "/dev/ttyUSB0"
CONF_SERIAL_PORT_1_RATE = "115200"
...
class motor_process(object):
    ...
    def open_serial(self):
        ...
        try:
            self.serial_port_1 = serial.Serial(CONF_SERIAL_PORT_1, CONF_SERIAL_PORT_1_RATE, timeout=1)
            self.serial_port_1.write(b'help\n\r')
        except:
            self.serial_port_1 = None
    ...
    def loop(self):
        ...
        while self.running:
            ...
            #       SERIAL ttyUSB0 
            #
            #if self.serial_port_1 != None and time() > self.serial_port_1_lastRead + 0:
            if self.serial_port_1 != None :

                self.serial_port_1_lastRead = time()

                try:
                    if self.serial_port_1.in_waiting > 0:

                        line = self.serial_port_1.readline().decode('utf-8').rstrip()
                        print(self.head +"Serial : "+ line)
...
#                        pi.set_servo_pulsewidth(self.data[i].gpio, 0)                   # Send to GPIO
                ...
                # traitement de la consigne pour les servos ST3215
                    ...
                    if self.data[i].current != self.data[i].consigne :
                        ...
                        self.serial_port_1.write(cmd.encode('utf-8'))
